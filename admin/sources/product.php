...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'],300, 200,_upload_sanpham,$file_name,1);									
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
		...
		$data['tinhtrang'] = $_POST['tinhtrang'];
			$data['phuongtien'] = $_POST['phuongtien'];
				$data['chinhsach'] = magic_quote($_POST['chinhsach']);
		...
		$data['ngaysua'] = time();$data['sao'] = $_POST['sao'];
		...
		foreach ($config['lang'] as $key => $value) {
			...
			$data['video'.$key] = magic_quote($_POST['video'.$key]);				
		}		
		$d->setTable('product');
		$d->setWhere('id', $id);
		if($d->update($data))
	            ...
	            for($i=0;$i<count($_FILES['files']['name']);$i++){
	            	if($_FILES['files']['name'][$i]!=''){
						if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
						{
							...
							$data1['photo'] = $photo;
							$data1['thumb'] = create_thumb($data1['photo'], 80, 100, _upload_hinhthem,$file_name,1);	
		...
		$data['chinhsach'] = magic_quote($_POST['chinhsach']);
				$data['thanhtoan'] = magic_quote($_POST['thanhtoan']);
			$data['tinhtrang'] = $_POST['tinhtrang'];
		$data['tag'] = $_POST['tag'];	$data['sao'] = $_POST['sao'];
			$data['phuongtien'] = $_POST['phuongtien'];
				$data['thoigian'] = $_POST['thoigian'];
					$data['khoihanh'] = $_POST['khoihanh'];
		$data['giacu'] = $_POST['giacu'];
...
function save_danhmuc(){
	...
	if($id){		
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){
			$data['photo'] = $photo;
			$data['thumb'] = create_thumb($data['photo'], 370, 250, _upload_sanpham,$file_name,1);	
			...
			if($photo1 = upload_image("file2", _format_duoihinh, _upload_sanpham,$file_name1)){
			$data['file'] = $photo1;				
			$d->setTable('product_danhmuc');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_sanpham.$row['file']);	
			}
		}	
		$data1['type'] = $_POST['type'];
		$data['tenkhongdau'] = changeTitle($_POST['ten']);

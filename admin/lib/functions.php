...
if ($to > $totalPages) { $to = $totalPages; }
	for($j = $from; $j <= $to; $j++) {
	   if ($j == $currentPage) $links = $links . "<li><a class='active' >{$j}</a></li>";		
	   else{				
		$qt = $url. "&p={$j}";
		$links= $links . "<li><a href = '{$qt}'>{$j}</a></li>";
	   } 	   
	} //for
	return '<ul>'.$firstLink.$links.$lastLink.'</ul>';
}
...
$image_ext_tem=explode('.', $image_url);
$image_ext = trim(strtolower(end($image_ext_tem)));

<script language="javascript">  
  ...
  {       
    ...
    window.location ="index.php?com=news&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?><?php if($_REQUEST['type']!='') echo"&type=".$_REQUEST['type']; ?>&id_item="+a.value;  
    return true;
  }
...
<?php
  function get_main_item()
    {
      ...
      $str='
        <select id="id_item" name="id_item" onchange="select_onchange()" class="main_select select_danhmuc">
        <option value="0">Chọn danh mục 1</option>
        ';
  ...
  function get_main_list()
    {
      $sql_huyen="select * from table_news_list where hienthi=1 and  id_item='".$_REQUEST['id_item']."' order by stt,id desc ";
      $result=mysql_query($sql_huyen);
      $str='
...
<?php if($_GET['type']=='contrung' || $_GET['type']=='kiemsoat' || $_GET['type']=='sanpham' ) { ?>
...
<?php } ?>
 ...
 <?php if($_GET['type']!='sanpham' and $_GET['type']!='huongdan') { ?>
 ...
 <?php if($_GET['type']!='chinhsach' and $_GET['type']!='sanpham') { ?>
        <div class="formRow">
            <label>Mô tả </label>
            <div class="formRight">
                <textarea   rows="8" cols="" title="Viết mô tả ngắn bài viết"  class="tipS"  name="mota<?=$key?>" id="mota<?=$key?>"><?=@$item['mota'.$key]?></textarea>
